...
std::unique_ptr<Resource> Resource::create_resource(category_t category,
	...
	case loader_policy_t::IN_MEMORY:
		return util::make_unique<InMemoryResource>(category, id, path, format);
	case loader_policy_t::DYNAMIC:
		return util::make_unique<DynamicResource>(category, id, path, format);
	default:
